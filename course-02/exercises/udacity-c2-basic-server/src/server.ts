...
if(!id || !make || !model || !type || !cost){
  return res.status(400)
  .send('Car type , name, model and cost is required.')
}

const new_car : Car ={
  make:make , type:type, id:id , cost:cost, model:model
};
cars.push(new_car);
return res.status(200)
.send(`You added car of ${id} ,${type}, ${model} and ksh ${cost}`)
})
